...
public class Agreculture_Servlet extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) {
		...
		if (path.equals("/login.do")) {
			...
			session.setAttribute("password", StrUserPassword);
			LoginTo loginTo = new LoginTo();
			System.out.println(session.getAttribute("name"));
			loginTo.setName((String) session.getAttribute("name"));
			loginTo.setPassword((String) session.getAttribute("password"));
		...
		//	session.setMaxInactiveInterval(30);
			...
			if(session.getAttribute("name")==null && session.getAttribute("paasword")==null) {
				try {
